...
> **💡 实现说明**：Metadata 数据实际存储在底层的 **MVStore** 中，`MetadataStore` 是服务层的封装，负责管理一致性协议（Gossip、Quorum、2PC）。
...
> **⚠️ 重要澄清**：NexKV 采用**组内全确认**架构（而非通用 Quorum）
> - **组内全确认**：在组内（5-10节点）和跨组（10-20节点）范围内实现**线性一致性**
> - **2PC 协议**：用于分布式事务，提供跨分片的原子性保证
> - **Gossip 协议**：用于组间和全局的最终一致性同步，**采用分级收敛机制**：
>   - **PriorityCritical**（2PC 决策）：立即转发，< 1秒收敛
>   - **PriorityHigh**（节点健康）：5秒周期，< 5秒收敛
>   - **PriorityNormal**（元数据同步）：10秒周期，< 10秒收敛
>
> **与通用 Quorum 的区别**：
> - 通用 Quorum：多数派确认（N/2+1），允许脑裂
> - NexKV 组内全确认：全员确认，无脑裂，真正的线性一致
>
...
> **🎯 结论**：通过树形分组 + 智能路由，NexKV 可以在保持强一致性的同时，支持 100-1000 个节点的大规模集群。
...
> **📦 MetadataStore** 是元数据管理的**服务层封装**，协调底层存储（MVStore）和一致性协议（Gossip、Quorum、2PC）。
...
> **⚠️ 注意**：Quorum 只能提供最终一致性，真正的强一致性需要 2PC
...
> **重要**: 这是最核心的架构理念,所有后续设计和实现都必须遵循这个原则。
