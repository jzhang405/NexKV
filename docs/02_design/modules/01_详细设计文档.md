...
```mermaid
flowchart TB
    subgraph Doc["详细设计文档结构"]
        direction TB

        P1["Part 1: 元数据层详细设计<br/>• 元数据存储层（MVStore）<br/>• 元数据同步层（Gossip + Quorum）<br/>• 元数据接口层（CRUD + 版本管理）"]

        P2["Part 2: 单机-分布式一体详细设计<br/>• 虚拟节点架构<br/>• 后台异步数据迁移<br/>• 树形分组（大规模集群支持）"]

        P3["Part 3: 关键实现细节<br/>• 版本号管理<br/>• 变更日志处理<br/>• 故障恢复机制"]
    end

    style Doc fill:#f8f9fa,stroke:#333,stroke-width:2px
    style P1 fill:#e3f2fd,stroke:#333,stroke-width:1px
    style P2 fill:#fff3e0,stroke:#333,stroke-width:1px
    style P3 fill:#e8f5e9,stroke:#333,stroke-width:1px
```
...
```mermaid
flowchart TB
    subgraph Layer1["Layer 1 内部三大核心组件"]
        direction TB

        subgraph Storage["元数据存储层（持久化）"]
            MV["MVStore<br/>━━━━━━━━━━━━<br/>• 内存缓存（sync.Map）<br/>• 磁盘持久化（WAL）<br/>• MVCC 支持（多版本并发控制）<br/>• 嵌入式部署（零外部依赖）<br/><br/>存储内容：<br/>• 分片元数据（分片ID、副本列表、主副本节点、版本号）<br/>• 节点元数据（节点地址、状态、容量、负载信息）"]
        end

        subgraph Sync["元数据同步层（一致性）"]
            direction LR
            Gossip["Gossip 协议<br/>━━━━━━━━<br/>• 最终一致性<br/>• 周期性扩散<br/>• 随机选点<br/>• 抗网络波动"]
            TwoPC["组内全确认（2PC）<br/>━━━━━━━━━━<br/>• 线性一致性<br/>• 组内全员确认<br/>• 阻塞等待<br/>• 无脑裂风险"]

            GossipScene["应用场景：<br/>• Gossip：普通变更<br/>  （节点状态更新、负载信息刷新）<br/>• 2PC：关键变更<br/>  （分片创建、主副本切换、节点加入）"]
        end

        subgraph API["元数据接口层（对外服务）"]
            direction LR
            CRUD["CRUD 接口<br/>━━━━━━━<br/>• CreateMetadata<br/>• UpdateMetadata<br/>• DeleteMetadata<br/>• QueryMetadata"]
            Version["版本管理<br/>━━━━━━━<br/>• 版本号递增<br/>• 变更日志缓存<br/>• 增量同步<br/>• 冲突检测"]
        end
    ...
    style Layer1 fill:#f8f9fa,stroke:#333,stroke-width:2px
    style Storage fill:#e3f2fd,stroke:#333,stroke-width:1px
    style Sync fill:#fff3e0,stroke:#333,stroke-width:1px
    style API fill:#e8f5e9,stroke:#333,stroke-width:1px
    style MV fill:#ffffff,stroke:#333,stroke-width:1px
    style Gossip fill:#ffffff,stroke:#333,stroke-width:1px
    style TwoPC fill:#ffffff,stroke:#333,stroke-width:1px
    style GossipScene fill:#ffffff,stroke:#333,stroke-width:1px,stroke-dasharray: 3 3
    style CRUD fill:#ffffff,stroke:#333,stroke-width:1px
    style Version fill:#ffffff,stroke:#333,stroke-width:1px
```
...
flowchart TB
    subgraph Tree["树形分组架构"]
        ...
        subgraph Groups["组节点"]
            ...
            G1["Group-1<br/>(10节点)<br/>┌───┐<br/>│2PC│<br/>└───┘<br/>组内强一致<br/>(全员同意)"]
            G2["Group-2<br/>(10节点)<br/>┌───┐<br/>│2PC│<br/>└───┘<br/>组内强一致<br/>(全员同意)"]
            G3["Group-3<br/>(10节点)<br/>┌───┐<br/>│2PC│<br/>└───┘<br/>组内强一致<br/>(全员同意)"]
            GN["... Group-N<br/>(10节点)<br/>┌───┐<br/>│2PC│<br/>└───┘<br/>组内强一致<br/>(全员同意)"]
        end
    ...
    style Tree fill:#f8f9fa,stroke:#333,stroke-width:2px
    style Root fill:#e1f5ff,stroke:#333,stroke-width:2px
    style Groups fill:#fff9c4,stroke:#333,stroke-width:1px
    ...
    style GN fill:#c8e6c9,stroke:#333,stroke-width:1px
```
...
| 集群规模 | 方案 | 确认数量 | 延迟 | 可行性 |
|---------|------|---------|------|--------|
| **100 节点** | 传统 2PC | 99 个确认 | ≈ 5-10 秒 | ❌ 不可行 |
| **100 节点** | 树形分组 | 10 组 × 10 节点 = 9 个确认 | ≈ 50ms | ✅ 可行 |
| **1000 节点** | 传统 2PC | 999 个确认 | ≈ 分钟级 | ❌ 不可行 |
| **1000 节点** | 树形分组 | 100 组 × 10 节点 = 9 个确认 | ≈ 50ms | ✅ 可行 |
...
> **ADR**: `docs/02_design/decisions/005_external_kv_engine_selection.md`
...
- ✅ 双层缓存架构（内存 + MVStore）
- ✅ 已验证稳定（`host_manager.go`）

---

### 7.3 External KV：Bf-Tree
...
flowchart TB
    subgraph "NexKV 双引擎架构"
        ...
        subgraph "Layer 1: Metadata KV<br/>元数据层"
            MetaAPI[MetadataStore<br/>强类型接口]
            MetaCache[双层缓存<br/>map + MVStore]
            MetaStorage[sync.Map<br/>O(1) 点查询]
            MetaPersist[WAL + 快照]

            MetaAPI --> MetaCache
            MetaCache --> MetaStorage
            MetaStorage --> MetaPersist
        end

        subgraph "Layer 2: External KV<br/>业务数据层"
            ExtAPI[ExternalStore<br/>通用 KV 接口]
            MiniPages[Mini-Page<br/>64B-4KB]
            BfTree[Bf-Tree<br/>Lock-free SMR]
            ExtPersist[WAL + SSTable]
...
**External KV 实现**（预计 4 个月）：
- Phase 1（1.5 月）：移植 Bf-Tree 核心（Rust → Go）
- Phase 2（1 月）：实现 Lock-free SMR
- Phase 3（1 月）：对接 WAL + SSTable
- Phase 4（0.5 月）：集成测试 + 性能验证
