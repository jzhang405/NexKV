...
| 提交阶段 | 时间节点 | 提交内容 | 评审重点 |
|----------|---------|---------|---------|
| 阶段1 | 第 1 周结束 | Phase 1-2 完成（单节点 + 多节点集群） | Daemon 进程管理、集群编排基础功能 |
| 阶段2 | 第 2 周结束 | Phase 3 完成（一致性协议测试） | Gossip/Quorum/2PC 测试用例完整性 |
| 阶段3 | 第 3 周中 | Phase 4 完成（故障注入测试） | FaultInjector 实现、故障恢复验证 |
| 阶段4 | 第 3 周结束 | Phase 5 完成（并发场景测试） | ConcurrencyTestRunner 实现、并发稳定性 |
| 阶段5 | 第 4 周中 | Phase 6 完成（性能测试） | MetricsCollector 实现、性能指标达标 |
| 最终提交 | 第 4 周结束 | 全量代码合并、文档定稿、CI 集成 | 代码规范、交付物完整性、CI 通过 |
