...
| **libp2p 集成复杂** | 中 | 低 | 1. **Week 0 POC**：正式开发前完成 1周 libp2p 基础通信 POC（局域网直连）⭐ v1.2<br/>2. **分阶段实现**：Week 1-2 同步 RPC → Week 3 Stream/Channel → Week 4 中间件<br/>3. **节点发现**：静态配置或 mDNS（局域网），无需 NAT 穿透 ⭐ v1.2 | Week 3 检查点 |
...
|------|-----------------|---------------|--------|
| **Week 0** | libp2p POC（局域网）⭐ v1.2 | - | POC 验证通过 |
...
**执行策略**：
1. ✅ **Week 0 POC**：完成 1周 libp2p 基础通信 POC（局域网直连）⭐ v1.2
2. ✅ **节点发现**：静态配置或 mDNS（局域网广播），无需 NAT 穿透 ⭐ v1.2
3. ✅ **接口层抽象**：保留接口层抽象，未来可扩展到公网环境
...
**Pre 文档 v1.2 修订完成（局域网部署环境）** ⭐

### 7.1 立即行动（Week 0）

1. ✅ **libp2p POC 验证（局域网）**（1周）⭐ v1.2
   - 验证局域网节点发现（mDNS 或静态配置）
   - 验证节点间直连通信
   - 性能基准测试（连接建立时间、吞吐量）
