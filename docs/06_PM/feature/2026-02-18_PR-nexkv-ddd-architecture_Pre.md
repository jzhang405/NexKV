...
### 4.1 里程碑时间表（30-32周）⭐ v1.6 修订

| 里程碑 | 时间 | 关键成果 | 可验证标准 | 阶段 | 状态 |
|--------|------|----------|------------|------|------|
| **M0** | 第0周 | 团队培训完成 ✅ | DDD + Go 泛型 + libp2p + Bf-Tree + Gossip + Quorum + 分片管理 培训通过 | 准备阶段 | ✅ **已完成** |
| **M1** | 第4周 | 基础设施层完成 | 16个接口单元测试通过（含 POC 验证）⭐ | 第一阶段 | 🔄 进行中 |
...
| 时间 | Bf-Tree 移植任务 | DDD Phase 任务 | 检查点 | 状态 |
|------|-----------------|---------------|--------|------|
| **Week 0** | — | 团队培训（DDD + Go 泛型 + libp2p + Bf-Tree + 一致性协议）⭐ | M0 里程碑 | ✅ **已完成** |
| **Week 1-4** | Bf-Tree 移植 Phase 1-2 | Phase 1 基础设施层（含 POC 验证）⭐ | M1 里程碑 | 🔄 进行中 |
...
**Pre 文档 v1.6 修订（M0 培训已完成，Phase 1 启动）** ⭐

### 7.1 立即行动（Week 0 + Week 1）⭐ v1.6 修订

**Week 0: 团队培训（3天）✅ 已完成** ⭐ v1.6
...
## 十、后续改进建议（非阻塞）⭐ v1.7 新增

> **来源**: BroadcastTracker Callback 评审经验总结（2026-02-21）
> **文档**: [`docs/06_PM/templates/pre-review-checklist.md`](../templates/pre-review-checklist.md)

### 10.1 接口设计补充

#### 10.1.1 Nil 参数行为定义

**当前状态**: ⚠️ 未明确（47 个接口中大部分未说明 nil 行为）

**改进建议**: 在后续 Phase 实施时，为每个接口补充 Nil 参数行为说明
...
type Transport interface {
    ...
    // 参数说明：
    //   - ctx: 上下文（不会为 nil）
    //   - addr: 监听地址（不会为 nil，格式："/ip4/0.0.0.0/tcp/0"）
    // 返回值：
    //   - error: 监听失败时返回错误（不会为 nil）
    Listen(ctx context.Context, addr string) error

    // Close 关闭传输层
    // 返回值：
    //   - error: 关闭失败时返回错误（不会为 nil）
    Close() error
}
```

**实施计划**: Phase 1-6 实施时逐步补充

---

#### 10.1.2 触发条件明确性

**当前状态**: ⚠️ 部分明确（如 Event.Channel 触发条件未完全说明）

**改进建议**: 为所有事件/回调接口补充触发条件
...
// 触发条件：
//   - 分片大小超过阈值（> 64MB）
//   - 分片写入 QPS 超过阈值（> 10K ops/sec）
//   - 管理员手动触发（Admin API）
// 不触发条件：
//   - 分片大小 < 32MB
//   - 分片正在迁移中
//   - 集群处于维护模式
OnSplit(ctx context.Context, event SplitEvent) error
```

**实施计划**: Phase 3-4 实施时补充

---

#### 10.1.3 "仅触发一次"机制

**当前状态**: ⚠️ 未说明（如 MetadataStore.Watch 是否保证仅触发一次？）

**改进建议**: 为所有事件监听接口说明"仅触发一次"保证
...
**当前状态**: ⚠️ 未明确（测试计划中缺少边界场景）

**改进建议**: 补充以下边界场景测试

| 测试场景 | 说明 | 适用接口 | 优先级 |
|---------|------|---------|--------|
| **空输入测试** | targets=[]、key=""、value=nil | 所有接口 | P0 |
| **全部失败测试** | 所有节点不可达 | Transport、Replicator | P0 |
| **并发安全测试** | 并发调用同一接口 | 所有接口 | P0 |
| **顺序依赖测试** | 先 Watch 后 Put | MetadataStore | P1 |
...
- **影响**: 高
- **缓解措施**: 实现幂等性或使用标志位机制
- **适用接口**: MetadataStore.Watch、ShardEvent.OnSplit

**风险 2: Nil 参数处理**
- **可能性**: 低
- **影响**: 中
- **缓解措施**: 在接口定义中明确 nil 行为
- **适用接口**: 所有接口
...
| 改进项 | 优先级 | 实施阶段 | 预计时间 |
|--------|--------|---------|---------|
| **Nil 参数行为定义** | P1 | Phase 1-6 | 4h（47个接口） |
| **触发条件明确性** | P1 | Phase 3-4 | 2h（14个事件接口） |
| **"仅触发一次"机制** | P2 | Phase 2-3 | 2h（含实现） |
| **边界场景测试** | P0 | Phase 1-6 | 6h（含测试用例） |
| **风险评估补充** | P1 | Phase 1 | 1h |

**总预计时间**: 15h（分散在 Phase 1-6 中）

---

### 10.5 评审检查清单应用

**已创建**: [`docs/06_PM/templates/pre-review-checklist.md`](../templates/pre-review-checklist.md)
...
**v1.7 修订说明**:
- ✅ 添加"后续改进建议"章节（基于 BroadcastTracker Callback 评审经验）
- ✅ 不影响已批准的内容（仅作为后续实施参考）
- ✅ 提供具体的改进计划和预计时间

---

**文档版本**: v1.7 ⭐（M0 培训完成，Phase 1 进行中，补充改进建议）
**创建日期**: 2026-02-18
**最后更新**: 2026-02-21
**批准日期**: 2026-02-18
**维护者**: NexKV 开发团队
**状态**: ✅ M0 已完成，Phase 1 进行中

---

## 九、版本历史

### v1.6（2026-02-19）⭐ **M0 培训完成，Phase 1 启动**

**修订原因**：M0 里程碑（Week 0 培训）已完成，启动 Phase 1 开发
...
**Phase 1 启动状态**：
- 🔄 Week 1-4：基础设施层开发 + POC 验证（进行中）
  - Week 1-2：Transport 接口实现 + 核心 POC 验证
  - Week 3-4：Codec 接口实现 + 扩展 POC 验证
...
|--------|------|------|------|
| **M0 状态** | 计划中 | ✅ 已完成 | 状态更新 ⭐ |
| **Phase 1 状态** | 待启动 | 🔄 进行中 | 状态更新 ⭐ |
| **Week 0 培训** | 计划中 | ✅ 已完成 | 状态更新 ⭐ |
| **培训材料链接** | 无 | 已添加 | 新增 ⭐ |
