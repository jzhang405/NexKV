...
- **现有问题**：
  ...
  - 纯内存模式实测性能：单线程 535K ops/sec，8 线程 548K ops/sec（扩展比仅 1.06x）
  - 对比目标架构（Lealone）：8 线程 2,074K ops/sec（扩展比 4.03x），差距 3.78x
...
**目标性能预期**（基于 Lealone 实测数据验证）：

| 线程数 | 目标吞吐量 (ops/sec) | 目标扩展比 | 参考架构 |
|--------|---------------------|-----------|----------|
| 1 | 600,000+ | 1.0x | - |
| 2 | 700,000+ | 1.4x | Lealone: 654K |
| 4 | 1,000,000+ | 2.0x | Lealone: 1,319K |
| 8 | 1,500,000+ | 2.8x | Lealone: 2,074K |
...
Lealone 参考性能:
  单线程: 514,928 ops/sec
  2线程: 653,918 ops/sec (扩展比 1.27x)
  4线程: 1,319,294 ops/sec (扩展比 2.56x)
  8线程: 2,073,742 ops/sec (扩展比 4.03x)
```

**关键发现**：
- 单线程性能基本一致（差距 < 1%）
- 多线程性能差距巨大（8 线程 3.78x）
- Lealone 的叶子级别锁定是其高并发性能的核心
...
| 对比分析 | Lealone vs NexKV | docs/10_benchmark/2026-03-18-memory-mode-perf/COMPARISON.md |
...
**参考数据** (Lealone 实测，1M 数据集)：

| 线程数 | 吞吐量 (ops/sec) | 平均延迟 (μs/op) | 扩展比 |
|--------|------------------|------------------|--------|
| 1 | 514,928 | 1.94 | 1.00x |
| 2 | 653,918 | 1.53 | 1.27x |
| 4 | 1,319,294 | 0.76 | 2.56x |
| 8 | 2,073,742 | 0.48 | 4.03x |
...
| 指标 | 当前值 | 目标值 | 提升幅度 | 参考依据 |
|------|--------|--------|----------|----------|
| 单线程 | 535K | 600K | +12% | 保守估计 |
| 2 线程 | 492K | 700K | +42% | Lealone 实测 654K |
| 4 线程 | 529K | 1,000K | +89% | Lealone 实测 1,319K |
| 8 线程 | 548K | 1,500K | +174% | Lealone 实测 2,074K |
| **扩展比** | **0.93x** | **2.8x** | **+201%** | 基于 Lealone 4.03x |
...
- `docs/10_benchmark/2026-03-18-memory-mode-perf/COMPARISON.md`

**测试工具**：
- NexKV: `cmd/btree_perf_mem/main.go`
- Lealone: `com.lealone.aose.benchmark.SimplePerfTest`
