...
│  └─────────┘           └─────────┘                   │
│                                                       │
│  • 新版本忽略 UDPPort 字段（backward compatible）         │
│  • 旧版本保留 UDPPort = 0（占位）                   │
│  • 使用 msgpack:"omitempty" 忽略零值                   │
│                                                       │
└─────────────────────────────────────────────────────────────┘
                    ↓
┌─────────────────────────────────────────────────────────────┐
...
type NodeAddress struct {
    ...
    UDPPort int    `msgpack:"udp_port,omitempty"` // 忽略零值
}
...
| 检查项 | 操作 | 验证方法 |
|--------|------|----------|
| **配置文件备份** | 迁移前备份所有配置文件 | `ls config.yaml.bak.*` |
| **序列化兼容性** | 测试新旧版本互操作 | 单元测试覆盖 |
| **节点通信** | 验证混合版本集群通信 | 集成测试覆盖 |
...
| 风险项 | 缓解措施 | 验证方法 |
|--------|----------|----------|
| **序列化不兼容** | 使用 `omitempty` 标签，零值忽略 | 单元测试：msgpack 序列化 |
| **节点无法启动** | 提供配置迁移脚本 | 测试：空 UDP 端口启动 |
| **集群通信失败** | 双版本兼容期，灰度发布 | 集成测试：混合版本通信 |
