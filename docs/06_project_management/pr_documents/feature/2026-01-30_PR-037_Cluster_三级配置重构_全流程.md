...
cluster:
  name: "nexkv-cluster"
  base_dir: "/var/tmp/nexkv"  # 可被 NEXKV_BASE_DIR 环境变量覆盖
...
// 示例：
// BaseDir: "/var/tmp/nexkv"
// HostID:  "host-1"
// 结果:    "/var/tmp/nexkv/host-1"
```
...
//
// 默认路径探索顺序（优先级从高到低）：
//   1. ~/.nexkv/data   - 用户主目录（默认，更安全）
//   2. ~/var/tmp/nexkv - 用户主目录（备选）
//   3. /var/tmp/nexkv   - 系统临时目录（最后选择）
//
// 如果所有路径都无法创建，返回错误
func GetBaseDir() (string, error) {
    ...
    candidates := []struct {
        ...
        {expandUser("~/.nexkv/data"), "用户主目录（默认）"},
        {expandUser("~/var/tmp/nexkv"), "用户主目录（备选）"},
        {"/var/tmp/nexkv", "系统临时目录（最后选择）"},
    }
...
2. 默认路径探索（按顺序尝试，全部失败则报错）：
   - ~/.nexkv/data（用户主目录，默认位置，最安全）
   - ~/var/tmp/nexkv（用户主目录，备选位置）
   - /var/tmp/nexkv（系统临时目录，最后选择，有清理风险）
...
Options:
  ...
  -m, --mode string         运行环境：dev/cluster（默认 cluster）
  -n, --hosts-num int       dev环境模拟hosts数目（默认 1）
...
| `--mode` | `-m` | 运行环境 | `cluster` | `dev` 模式支持本地多 Host 模拟 |
| `--hosts-num` | `-n` | 模拟 Host 数目 | `1` | 仅在 `dev` 模式下有效 |
...
| **运行时选项**（log-level, mode） | 命令行参数 | 通过命令行参数控制 |
...
#    - 否则按默认路径探索顺序查找（~/var/tmp/nexkv → /var/tmp/nexkv）
nexkvd
```
...
# 2. 开发环境：指定配置文件和日志级别
nexkvd --config ./config.yaml --log-level debug

# 3. 本地测试：dev 模式模拟 3 个 Host
nexkvd --mode dev --hosts-num 3 --log-level debug

# 4. 指定基础目录（通过环境变量）
export NEXKV_BASE_DIR=/var/tmp/nexkv
nexkvd
```
