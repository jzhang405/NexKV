...
func DetermineConfigPath(configFlag string) string {
    // 1. 优先使用命令行参数 --config
    ...
    baseDir := GetBaseDir() // 从环境变量或默认值获取
...
//
// 默认路径探索顺序（优先级从高到低）：
//   1. /var/tmp/nexkv    - 系统临时目录（重启后可能被清理）
//   2. ~/var/nexkv      - 用户主目录（持久化存储）
//   3. ./data            - 当前目录（开发环境兜底）
    ...
    for _, dir := range candidates {
        ...
        if _, err := os.Stat(dir); os.IsNotExist(err) {
            if err := os.MkdirAll(dir, 0755); err == nil {
                return dir // 创建成功，使用此目录
            }
        } else {
            return dir // 目录已存在，使用此目录
        }
    }
