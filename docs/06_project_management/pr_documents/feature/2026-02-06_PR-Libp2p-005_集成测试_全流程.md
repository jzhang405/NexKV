...
| 架构师评审状态 | ✅ **已批准** - 2026-02-06 |
| 预审批结果 | ✅ 通过（4轮评审完成：性能目标 + 架构迁移 + 连接超时 + 一致性协议 + MessagePack Payload） |
...
##### 3.3.4 MessagePack Payload 结构设计

> **📌 核心设计原则**：采用「结构化 Payload 定义 + 通用 []byte 传输」，使用 MessagePack 序列化，兼顾类型安全与扩展性。
>
> **⚡ 架构优化**（用户建议）：将 `Key`、`Value`、`Version` 从核心 Message 结构移到 Payload，让 Message 结构更简洁通用。

**设计对比**：

| 方案 | 类型安全 | 开发效率 | 扩展成本 | 调试难度 |
|------|----------|----------|----------|----------|
| **MessagePack + 结构化** | ✅ 编译期检查 | ✅ 高（原生结构体） | ✅ 低（仅加结构体） | ✅ 易（直接打印结构体） |
| 纯 []byte 裸解析 | ❌ 运行时 panic | ❌ 低（重复写解析逻辑） | ❌ 高（改解析逻辑） | ❌ 难（裸字节流） |
| Protobuf | ✅ 依赖 proto 定义 | ❌ 需编译 proto | ❌ 需修改 proto + 重新编译 | ✅ 易 |

**核心 Message 结构**（⚡ 优化后：移除 Key/Value/Version，改用 Payload）：

```go
// Message NexKV 协议消息定义（优化后）
type Message struct {
    // Type 消息类型
    Type MessageType
    // Seq 消息序号（单调递增）
    ...
    // HopCount 跳数（用于消息路由）
    HopCount uint8
    // Payload 扩展负载（存储 MessagePack 序列化后的结构化数据）
    // ⚡ 优化：Key、Value、Version 已移至各 Payload 结构体
    Payload []byte
}
```
...
| 字段 | 原位置 | 新位置 | 优点 |
|------|--------|--------|------|
| **Key** | Message.Key | PutPayload.Key, GetPayload.Key, DeletePayload.Key | ✓ 每种消息类型独立定义 |
| **Value** | Message.Value | PutPayload.Value, QuorumPayload.Value | ✓ 灵活可选，支持 omitempty |
| **Version** | Message.Version | PutPayload.Version, GossipPayload.VersionDelta | ✓ 版本号语义更清晰 |

**Payload 结构定义**（为每种 MessageType 定义专属结构体）：
...
// ⚡ 优化：Key、Value、Version 从 Message 移至此处
...
type QuorumPayload struct {
    ...
    Voter      string `msgpack:"voter,omitempty"`
    Decision   bool   `msgpack:"decision,omitempty"`
}
...
// PayloadTypeMap 维护 MessageType 与 Payload 结构体的映射
var PayloadTypeMap = map[MessageType]interface{}{
    ...
    MessageTypeSync:    &TwoPCPreparePayload{}, // 复用 MessageTypeSync
    MessageTypeAck:     &TwoPCCommitPayload{},  // 复用 MessageTypeAck
    MessageTypeNack:    &TwoPCRollbackPayload{}, // 复用 MessageTypeNack
...
func (m *Message) DecodePayload() (interface{}, error) {
    ...
    payloadProto, ok := PayloadTypeMap[m.Type]
    if !ok {
        return nil, errors.New("unsupported message type: " + m.Type.String())
    }

    // 2. 深拷贝 Payload 原型（避免覆盖映射表）
    payload := msgpack.Clone(payloadProto)
...
func TestMessageCompatibility(t *testing.T) {
    ...
    putPayload := &transport.PutPayload{
        Key:     []byte("test-key"),
    ...
    // 3. 验证消息类型已绑定
    assert.Equal(t, transport.MessageTypePut, msg.Type)
    ...
    // 6. 类型断言验证
    putDecoded, ok := decoded.(*transport.PutPayload)
    require.True(t, ok)
...
func TestQuorumPayloadEncoding(t *testing.T) {
    ...
    proposePayload := &transport.QuorumPayload{
        Phase:      "propose",
        ProposalID: "prop-001",
        Key:        "shard-001",
        Value:      []byte("metadata"),
    ...
    quorumDecoded, ok := decoded.(*transport.QuorumPayload)
    require.True(t, ok)
...
func TestTwoPCPayloadEncoding(t *testing.T) {
    ...
    twoPCDecoded, ok := decoded.(*transport.TwoPCPreparePayload)
    require.True(t, ok)
...
| 条件 | 优先级 | 状态 | 说明 |
|------|--------|------|------|
| ⚠️ Quorum 一致性测试 | 高 | **已完成** | TestQuorumConsistency, TestQuorumFailure |
| ⚠️ 2PC 全员应答测试 | 高 | **已完成** | TestTwoPhaseCommit, TestTwoPhaseCommitWithFailure, TestTwoPhaseCommitSingleFailure |
| 💡 Quorum vs 2PC 性能对比 | 中 | **已完成** | BenchmarkQuorumVs2PC |
...
| 第2轮 | 2026-02-06 | 👤 架构师 | 💡 建议：补充 2PC 全员应答测试 | 1. 新增 3.3.7 节：一致性协议测试（Quorum vs 2PC）<br/>2. 新增 3.5.13-3.5.14 节：一致性协议测试用例<br/>3. 包含：TestQuorumConsistency, TestTwoPhaseCommit, BenchmarkQuorumVs2PC | ✅ 已完成 |
| 第3轮 | 2026-02-06 | 👤 用户确认 | ℹ️ 不需要新旧架构兼容性 | 1. 移除 3.5.15-3.5.17 节（新旧架构消息兼容性验证）<br/>2. 统一使用 libp2p 架构 | ✅ 已完成 |
| 第4轮 | 2026-02-06 | 👤 用户建议 | ⚡ Message 结构优化 | 1. 新增 3.3.4 节：MessagePack Payload 结构设计<br/>2. 将 Key/Value/Version 从 Message 移至 Payload<br/>3. 新增 3.3.5 节：消息协议兼容性测试（Payload 编解码） | ✅ 已完成 |
| **最终批准** | 2026-02-06 | 👤 架构师 | ✅ **Pre 文档批准，同意进入开发阶段** | 所有批准条件已满足：<br/>1. ✅ 性能目标修订（两阶段）<br/>2. ✅ 架构迁移测试<br/>3. ✅ 连接超时测试<br/>4. ✅ 一致性协议测试（Quorum + 2PC）<br/>5. ✅ MessagePack Payload 结构设计<br/>6. ✅ 并发关闭、冷启动延迟、goleak 检测 | **可开始开发** |
| **批准条件** | | | 1. ⚠️ 性能目标修订（阶段 1 + 阶段 2）<br/>2. ⚠️ 架构迁移测试（向后兼容 + API 表面）<br/>3. ⚠️ 连接超时测试（超时场景处理）<br/>4. ⚠️ 一致性协议测试（Quorum + 2PC）<br/>5. ⚠️ MessagePack Payload 结构设计（Key/Value/Version 移至 Payload）<br/>6. 💡 建议补充：并发关闭、冷启动延迟、goleak 检测 | | |
