...
| 名称 | 全称 | 核心特征 | 典型场景 |
|------|------|----------|----------|
| **B+ Tree** | B+树 | 叶子节点链表串联，内部节点仅存 key | 数据库/分布式 KV 索引 |
| **B* Tree** | B*树 | 节点分裂时优先重分配，减少碎片 | 磁盘存储优化 |
| **Bε-Tree** | Bε树（ε-optimized） | 基于 ε 因子优化节点填充率 | 高内存利用率 KV |
| **BF+Tree** | Bloom Filter + B树 | B 树前置布隆过滤器，过滤无效查询 | 海量数据快速过滤 |
| **Bf-Tree** | Buffer-Friendly Tree | Mini-Page + Delta Chain + Promotion | 高并发读写优化（NexKV 选择） |

### 0.2 NexKV 选择 Bf-Tree 的理由

NexKV 选择微软的 **Bf-Tree（Buffer-Friendly Tree）**，原因：

| 维度 | B+ Tree | Bε-Tree | BF+Tree | **Bf-Tree（NexKV）** |
|------|---------|---------|---------|---------------------|
| **写入性能** | 中 | 中 | 中 | **高**（Mini-Page 增量写） |
| **读取性能** | 高 | 高 | 高（BF 过滤） | **高**（内存优先） |
| **范围查询** | ✅ | ✅ | ✅ | ✅ O(log N + M) |
| **并发控制** | 复杂 | 复杂 | 复杂 | **可简化**（RWMutex） |
| **持久化** | 需自研 | 需自研 | 需自研 | **WAL 可复用** |

### 0.3 对比实验目的

1. **建立性能基准**：标准 B+ 树 vs Bf-Tree MVP
2. **验证选型决策**：确认 Bf-Tree 优势场景
3. **发现优化方向**：借鉴优秀实现
...
| 特性 | B+ Tree | Bε-Tree | BF+Tree | Bf-Tree |
|------|---------|---------|---------|---------|
| **节点分层设计** | ✅ 内部仅 key，叶子链表 | ✅ 同 B+ | ✅ 同 B+ | ✅ 同 B+ |
| **CRUD 操作** | ✅ | ✅ | ✅ | ✅ |
| **范围查询** | ✅ 叶子链表遍历 | ✅ 同 B+ | ✅ 同 B+ | ✅ O(log N + M) |
| **阶数配置** | ✅ 自定义 m | ✅ m + ε 因子 | ✅ 同 B+ | ✅ 多级页面 |
| **错误处理** | ✅ | ✅ | ✅ | ✅ |
...
| 特性 | B+ Tree | Bε-Tree | BF+Tree | Bf-Tree |
|------|---------|---------|---------|---------|
| **并发安全** | ⚠️ 需自行加锁 | ⚠️ 需自行加锁 | ⚠️ 需自行加锁 | ✅ RWMutex/SMR |
| **迭代器** | ✅ | ✅ | ✅ | ✅ |
| **批量操作** | ⚠️ 需自研 | ⚠️ 需自研 | ⚠️ 需自研 | ✅ Mini-Page |
| **序列化** | ⚠️ 需自研 | ⚠️ 需自研 | ⚠️ 需自研 | ✅ WAL |
| **统计指标** | ⚠️ 需自研 | ✅ ε 命中率 | ✅ BF 误判率 | ✅ 分裂/提升 |

### 3.3 高级特性对比（可选特性）

| 特性 | B+ Tree | Bε-Tree | BF+Tree | Bf-Tree |
|------|---------|---------|---------|---------|
| **持久化** | ⚠️ 需自研 | ⚠️ 需自研 | ⚠️ 需自研 | ✅ WAL + Snapshot |
| **前缀压缩** | ⚠️ 可选 | ⚠️ 可选 | ⚠️ 可选 | ⚠️ 可选 |
| **TTL 支持** | ⚠️ 需自研 | ⚠️ 需自研 | ⚠️ 需自研 | ⚠️ 可选 |
| **增量更新** | ❌ | ❌ | ❌ | ✅ **Mini-Page** |
| **概率提升** | ❌ | ❌ | ❌ | ✅ **Promotion** |
| **快速过滤** | ❌ | ❌ | ✅ **布隆过滤器** | ⚠️ 可选 |
...
| NexKV 需求 | 优先级 | B+ Tree | Bε-Tree | BF+Tree | **Bf-Tree** |
|-----------|--------|---------|---------|---------|-------------|
| 高吞吐写入 | P0 | ⚠️ | ⚠️ | ⚠️ | ✅ Mini-Page |
| 低延迟读取 | P0 | ✅ | ✅ | ✅ | ✅ 内存优先 |
| 范围扫描 | P0 | ✅ | ✅ | ✅ | ✅ |
| 持久化 | P0 | ⚠️ | ⚠️ | ⚠️ | ✅ WAL 复用 |
| 并发安全 | P0 | ⚠️ | ⚠️ | ⚠️ | ✅ RWMutex |
| 分布式适配 | P1 | ⚠️ | ⚠️ | ⚠️ | ✅ |

**结论**：Bf-Tree 最匹配 NexKV 需求
