...
2. **类型安全**
   - 引入泛型 `Future[T]` 替代 `TaskResult[any]`
   - 提供 `SubmitWithResult[T]()` 辅助函数

3. **命名统一**
   - 文件重命名：`taskpool_*` → `executor_*`
   - 类型重命名：`AntsTaskPoolProvider` → `AntsTaskExecutorProvider`

4. **Deadcode 清理**
   - 删除 13 个未使用文件
   - 删除 `DomainEvent` 接口及 30 个方法实现
   - 保留事件结构体（被 service 包导出）

#### 设计决策记录

**为什么使用泛型辅助函数而非泛型接口？**
- Go 不支持接口方法上的类型参数
- 避免接口爆炸（每个类型参数组合都生成新接口）
- 保持向后兼容，不修改现有接口

**为什么删除 DomainEvent 接口？**
- 接口方法从未被使用
- 事件溯源功能尚未实现
- 保留事件结构体供未来使用
...
**新增章节**：
- ✅ 已完成重构 - concurrency 包接口简化

**主要成果**：
- 接口数量：9 → 3（-67%）
- 代码减少：18,935 → 10,280 行（-46%）
- Deadcode 清理：665 → 392（-41%）
- 所有测试通过，性能稳定

**设计亮点**：
- 泛型 `Future[T]` 实现类型安全
- 小接口组合符合 SOLID 原则
- 命名统一规范（taskpool → executor）

**相关文档**：
- 详细报告：`docs/refactoring/2025-03-01-interface-simplification.md`
