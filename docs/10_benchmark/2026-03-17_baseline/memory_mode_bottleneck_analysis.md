...
> **⚠️ 重要更新**：本报告基于 1000 keys 测试，后续使用 1M keys 验证发现部分结论需要调整。详见 [1M Keys 性能验证报告](./1m_keys_perf_verification.md)
...
| 瓶颈类型 | CPU 占比 | 样本数 | 优化优先级 | 1M keys 验证 |
|----------|---------|--------|-----------|-------------|
| **GC 相关** | 45-50% | ~240-267 | P0 | ✅ 46% （准确） |
| **栈展开** | ~12% | ~64 | P1 | ✅ 持平 |
| **应用层** | ~2% | ~11 | P1 | ⚠️ 实际 1.1% （高估） |
| **defer 开销** | 6.90% | ~37 | P0 | ⚠️ 实际 8.58% （低估） |
| **其他** | ~36% | ~192 | P2 | - |

### 核心发现（已验证）

1. **GC 是最大瓶颈**：占总 CPU 时间的 45-50% ✅ **准确**
2. **应用层优化空间有限**：实际仅占 1.1%（vs 预估 2%）
3. **defer 开销被低估**：实际占 8.58%（vs 预估 6.90%）

### 优化优先级调整（基于 1M keys 验证）

| 优先级 | 优化项 | 1000 keys 预估 | 1M keys 验证 | 调整 |
|--------|--------|---------------|-------------|------|
| **P0** | 减少 defer | P1 → **P0** ⬆️ | 8.58% | 提升 |
| **P0** | 对象池 | P0 | P0 | 保持 |
| **P1** | 优化 CloneShallow | P1 → **P2** ⬇️ | 0.52% | 降低 |
| **P1** | 优化 GetParentRef | P1 → **P2** ⬇️ | 0.16% | 降低 |
...
| 函数 | CPU 占比 | 单次开销 | 调用频率 | 优先级 |
|------|---------|---------|----------|--------|
| **PageInfo.CloneShallow** | 0.88% | ~120 ns | 每次 Set（3-5 次） | P1 |
| **PageInfo.CloneDeep** | 0.73% | ~630 ns | CAS 成功后 1 次 | P1 |
| **PageInfo.GetParentRef** | 0.51% | ~25 ns | 每次 Set（3-5 次） | P1 |
| **LeafPage.search** | ~0.24% | ~80 ns | 每层搜索 | P2 |
| **InternalPage.FindChildRef** | ~0.12% | ~40 ns | 内部节点查找 | P2 |
| **应用层总计** | **~2.5%** | - | - | - |
...
│   ├── getRoot()
│   └── searchLevel() (第 3 层)
│       └── InternalPage.FindChildRef() (~40 ns)
├── copyPathShallow (第 4 层)
│   └── PageInfo.CloneShallow() (第 5-6 层, ~120 ns)
│       ├── 分配 PageInfo (~50 ns)
│       ├── NewPageLock() (~20 ns)
│       ├── GetParentRef() (~30 ns)
│       └── 原子 Store (~20 ns)
└── modifyPath (第 8 层)
    ├── LeafPage.search() (~80 ns)
    └── page.Insert() (第 9-10 层)
        └── 可能触发分裂
```
...
**关键发现**：
1. **应用层仅占 2.5% CPU**：相对于 GC 的 45-50%，应用层优化空间有限
2. **CloneShallow 是热点**：每次 Set 调用 3-5 次，总计 ~360-600 ns
3. **GetParentRef 锁开销**：每次调用 ~25 ns，但调用频繁（3-5 次/Set）
4. **调用深度深**：10 层调用增加了栈展开开销（GC 扫描栈帧）
...
| 操作 | 调用次数 | 单次开销 | 总计 |
|------|---------|---------|------|
| FindChildRef | 2-3 次 | 40 ns | 80-120 ns |
| CloneShallow | 3-5 次 | 120 ns | 360-600 ns |
| GetParentRef | 3-5 次 | 25 ns | 75-125 ns |
| LeafPage.search | 1 次 | 80 ns | 80 ns |
| CloneDeep（CAS 成功） | 1 次 | 630 ns | 630 ns |
| **总计（不含 CAS 失败重试）** | - | - | **~1.2-1.5 μs** |

**占总延迟比例**：
- 应用层：1.5 μs / 5.34 μs = **28%**
- GC + runtime：**72%**
...
> **验证日期**：2026-03-17
> **验证目的**：使用 1M keys 验证 1000 keys 分析的准确性
> **详细报告**：[1M Keys 性能验证报告](./1m_keys_perf_verification.md)

### 6.1 关键结论更新

| 之前结论（1000 keys） | 验证结果（1M keys） | 准确性 |
|---------------------|-------------------|--------|
| **GC 是主要瓶颈（45-50%）** | ✅ GC 占 46% | **准确** |
| **应用层占 2.5%** | ⚠️ 实际占 1.1% | **高估 2.3x** |
| **defer 占 6.90%** | ⚠️ 实际占 8.58% | **低估 24%** |
| **CloneShallow 是热点（0.88%）** | ❌ 实际 0.52% | **高估 1.7x** |
| **GetParentRef 是瓶颈（0.51%）** | ❌ 实际 0.16% | **高估 3.2x** |

### 6.2 优化优先级调整

**之前基于 1000 keys 的优先级**：
1. PageInfo 对象池（P0）
2. atomic.Value 替代锁（P0）
3. 减少 defer 使用（P1）

**验证后基于 1M keys 的优先级**：
1. **减少 defer 使用（P0）** ⬆️ defer 占 8.58%
2. **PageInfo 对象池（P0）** GC 占 46%
3. atomic.Value 替代锁（P2）⬇️ GetParentRef 仅 0.16%
...
1. **样本量增加提升准确性**：
   - 1M keys 测试收集了 3101 个样本（vs 533）
   - 统计分析更可靠

2. **应用层优化空间有限**：
   - 应用层仅占 1.1% CPU（vs 预估 2.5%）
   - 优化 CloneShallow、GetParentRef 意义不大

3. **defer 开销被低估**：
   - tryDeferToSpanScan 从 6.90% → 8.58%（+24%）
   - defer 优化优先级应提升至 P0
...
**立即实施（P0）**：
1. 减少 defer 使用（热点 defer：setWithCAS、GetParentRef、finalizeDeepClone）
2. PageInfo 对象池（减少 GC 压力）

**放弃的优化（P2）**：
- 优化 CloneShallow（占比仅 0.52%）
- 优化 GetParentRef（占比仅 0.16%）
- 优化 CloneDeep（占比仅 0.15%）
