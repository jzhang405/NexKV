...
type BroadcastProgress struct {
	...
	// Callback 机制（v1.4）
	callback                  service.BroadcastListener // 回调接口（可选）
...
func (t *BroadcastProgress) SetCallback(cb service.BroadcastListener) {
	t.mu.Lock()
	defer t.mu.Unlock()
	t.callback = cb
}
...
func (t *BroadcastProgress) buildStatsLocked() service.BroadcastStats {
...
// OnSuccess 成功回调
func (b *ProgressBuilder) OnSuccess(fn func(peer model.PeerID, resp model.Message, stats service.BroadcastStats)) *ProgressBuilder {
...
// OnFailure 失败回调
func (b *ProgressBuilder) OnFailure(fn func(peer model.PeerID, err error, stats service.BroadcastStats)) *ProgressBuilder {
	if b.progress.callback == nil {
		b.progress.callback = &builderListener{progress: b.progress}
	}
...
func (b *ProgressBuilder) OnMajority(fn func(stats service.BroadcastStats)) *ProgressBuilder {
	if b.progress.callback == nil {
		b.progress.callback = &builderListener{progress: b.progress}
	}
...
func (b *ProgressBuilder) OnComplete(fn func(stats service.BroadcastStats)) *ProgressBuilder {
	if b.progress.callback == nil {
		b.progress.callback = &builderListener{progress: b.progress}
	}
...
func (l *builderListener) OnSuccess(peer model.PeerID, resp model.Message, stats service.BroadcastStats) {
	if l.onSuccess != nil {
		l.onSuccess(peer, resp, stats)
	}
}

func (l *builderListener) OnFailure(peer model.PeerID, err error, stats service.BroadcastStats) {
	if l.onFailure != nil {
		l.onFailure(peer, err, stats)
	}
}

func (l *builderListener) OnMajority(stats service.BroadcastStats) {
	if l.onMajority != nil {
		l.onMajority(stats)
	}
}

func (l *builderListener) OnComplete(stats service.BroadcastStats) {
	if l.onComplete != nil {
		l.onComplete(stats)
	}
}
