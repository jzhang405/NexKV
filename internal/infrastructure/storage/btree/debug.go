...
func (b *BTree) AssertInvariants() error {
	rootPI := b.RootPage().GetPageInfo()
	if rootPI == nil {
		return errpkg.Wrapf(ErrBTreeDebugError, "btree: nil root")
	}
...
func (b *BTree) assertLeafInvariants(pageID model.PageID, minKey, maxKey []byte) error {
	leaf, err := b.storage.GetLeafPage(pageID)
	if err != nil {
		return errpkg.Wrapf(err, "btree: get leaf %d", pageID)
	}
	count := leaf.Count()
	for i := 1; i < count; i++ {
		...
		if string(prev) >= string(curr) {
			return errpkg.Wrapf(ErrBTreeDebugError, "btree: leaf %d: key[%d]=%q >= key[%d]=%q",
				pageID, i-1, prev, i, curr)
		}
	}
	if minKey != nil && count > 0 && string(leaf.GetKey(0)) < string(minKey) {
		return errpkg.Wrapf(ErrBTreeDebugError, "btree: leaf %d: first key %q < min key %q", pageID, leaf.GetKey(0), minKey)
	}
	if maxKey != nil && count > 0 && string(leaf.GetKey(count-1)) > string(maxKey) {
		return errpkg.Wrapf(ErrBTreeDebugError, "btree: leaf %d: last key %q > max key %q", pageID, leaf.GetKey(count-1), maxKey)
	}
	return nil
}
...
func (b *BTree) assertInternalInvariants(pageID model.PageID, minKey, maxKey []byte) error {
	node, err := b.storage.GetNodePage(pageID)
	if err != nil {
		return errpkg.Wrapf(err, "btree: get node %d", pageID)
	}
	count := node.Count()
	for i := 1; i < count; i++ {
		...
		curr := node.GetKey(i)
		if string(prev) >= string(curr) {
			return errpkg.Wrapf(ErrBTreeDebugError, "btree: node %d: key[%d]=%q >= key[%d]=%q",
				pageID, i-1, prev, i, curr)
		}
	}
	if node.ChildCount() != count+1 {
		return errpkg.Wrapf(ErrBTreeDebugError, "btree: node %d: child count %d != key count %d + 1",
