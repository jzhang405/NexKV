...
func (h *nodePageHandle) ReplaceChild(idx int, newChildID model.PageID) (NodePage, error) {
	count := h.Count()
	if idx < 0 || idx > count {
		return nil, errpkg.Wrapf(ErrBTreeValidationError, "btree: node replace child: index %d out of range [0, %d]", idx, count)
	}
...
func (h *nodePageHandle) InsertChild(idx int, splitKey []byte, left, right model.PageID) (NodePage, error) {
	count := h.Count()
	if idx < 0 || idx > count {
		return nil, errpkg.Wrapf(ErrBTreeValidationError, "btree: node insert child: index %d out of range [0, %d]", idx, count)
	}
...
func (h *nodePageHandle) RemoveChild(idx int) (NodePage, error) {
	count := h.Count()
	if idx < 0 || idx > count {
		return nil, errpkg.Wrapf(ErrBTreeValidationError, "btree: node remove child: index %d out of range [0, %d]", idx, count)
	}

	if count == 1 {
		return nil, errpkg.Wrapf(ErrBTreeValidationError, "btree: remove child: cannot remove only entry, node would be empty")
	}
...
func (h *nodePageHandle) Validate() error {
	...
	if h.ChildCount() != count+1 {
		return errpkg.Wrapf(ErrBTreeValidationError, "btree: node validate: child count %d != key count %d + 1", h.ChildCount(), count)
	}
	return nil
}
