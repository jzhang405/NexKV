...
func (cfg *btreeConfig) buildTxManager(storage mvcc.StorageBackend, lobMgr mvcc.LOBManager, lobFileMgr mvcc.LOBFileManager) mvcc.TxManager {
	if cfg.txMgr != nil {
		return cfg.txMgr
	}
	if lobMgr != nil {
		return mvcc.NewTxManagerWithLOB(storage, cfg.tsGen, lobMgr, lobFileMgr)
	}
