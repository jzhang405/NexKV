...
func (s *PageSerializer) Serialize(ptr unsafe.Pointer, pageLength int) ([]byte, error) {
	...
	if pageLength < MinPagePayload || pageLength > MaxPagePayload {
		return nil, errpkg.Wrap(ErrInvalidPageLength,
			fmt.Sprintf("page_serializer: invalid pageLength %d (range [%d,%d])", pageLength, MinPagePayload, MaxPagePayload))
	}
...
func (s *PageSerializer) Deserialize(data []byte, dst unsafe.Pointer) (int, error) {
	...
	if len(data) < MinDiskPageSize || len(data) > MaxDiskPageSize {
		return 0, errpkg.Wrap(ErrChunkFormatError,
			fmt.Sprintf("page_serializer: invalid data len %d (range [%d,%d])", len(data), MinDiskPageSize, MaxDiskPageSize))
	}
	if dst == nil {
	...
	if pageType != offheap.PageTypeIndex && pageType != offheap.PageTypeLeaf {
		return 0, errpkg.Wrap(ErrChunkFormatError, fmt.Sprintf("page_serializer: invalid pageType %d", pageType))
	}
