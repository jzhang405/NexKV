...
func ParseTxPrepareEntry(e *service.WALEntry) (txID uint64, entries []TxPrepareParsedEntry, err error) {
	if e.Type != service.WALTypeTxPrepare {
		return 0, nil, errpkg.Wrapf(errpkg.ErrMVCCTxPrepareCorrupted, "not a TxPrepare entry")
	}
	if len(e.Key) < 8 {
		return 0, nil, errpkg.Wrapf(errpkg.ErrMVCCTxPrepareCorrupted, "TxPrepare: key too short")
	}
	txID = binary.BigEndian.Uint64(e.Key[0:8])

	buf := e.Value
	if len(buf) < 4 {
		return 0, nil, errpkg.Wrapf(errpkg.ErrMVCCTxPrepareCorrupted, "TxPrepare: value too short")
	}
	...
	for i := uint32(0); i < keyCount; i++ {
		if offset+4 > len(buf) {
			return 0, nil, errpkg.Wrapf(errpkg.ErrMVCCTxPrepareCorrupted, "TxPrepare: truncated at key %d", i)
		}
		keyLen := binary.BigEndian.Uint32(buf[offset:])
		offset += 4
		if offset+int(keyLen) > len(buf) {
			return 0, nil, errpkg.Wrapf(errpkg.ErrMVCCTxPrepareCorrupted, "TxPrepare: truncated key data")
		}
		key := string(buf[offset : offset+int(keyLen)])
		offset += int(keyLen)

		if offset+12 > len(buf) {
			return 0, nil, errpkg.Wrapf(errpkg.ErrMVCCTxPrepareCorrupted, "TxPrepare: truncated at entry header")
		}
		...
		if offset+int(oldValLen) > len(buf) {
			return 0, nil, errpkg.Wrapf(errpkg.ErrMVCCTxPrepareCorrupted, "TxPrepare: truncated oldVal")
		}
		...
		if offset+5 > len(buf) {
			return 0, nil, errpkg.Wrapf(errpkg.ErrMVCCTxPrepareCorrupted, "TxPrepare: truncated at newVal header")
		}
		newFlag := buf[offset]
		offset++
		...
		if offset+int(newValLen) > len(buf) {
			return 0, nil, errpkg.Wrapf(errpkg.ErrMVCCTxPrepareCorrupted, "TxPrepare: truncated newVal")
		}
