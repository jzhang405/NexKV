...
// TestDispatcherStress 压力测试：大量消息
func TestDispatcherStress(t *testing.T) {
	...
	handler := &mockHandler{
		handleDelay: 1 * time.Millisecond, // 模拟慢处理
	}
	config := &DispatcherConfig{
		WorkerCount:   4,
		QueueSize:     1000,
	...
	var wg sync.WaitGroup
	for i := 0; i < connCount; i++ {
		addr := fmt.Sprintf("192.168.1.%d:9211", i)
		msgChan := make(chan MsgFrame, msgsPerConn)
		...
		go func(ch chan<- MsgFrame) {
			defer wg.Done()
			for j := 0; j < msgsPerConn; j++ {
				ch <- newTestMsgFrame(uint64(j), uint64(j), types.MessageTypeGet)
			}
			close(ch)
		}(msgChan)
	}
	...
	t.Logf("Stress test: Processed=%d, Dropped=%d, Total=%d",
		stats.MsgCount, stats.DropCount, totalMsgs)
	...
	// 2. 处理 + 丢弃 = 总数（或接近总数）
	sum := stats.MsgCount + stats.DropCount
	if sum < uint64(totalMsgs*90/100) {
		t.Errorf("Processed + Dropped (%d) < 90%% of Total (%d)", sum, totalMsgs)
	}
}
