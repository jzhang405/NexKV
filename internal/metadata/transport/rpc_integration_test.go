...
// TestRPCIntegration_HighConcurrency 测试高并发场景（100+ 并发）
func TestRPCIntegration_HighConcurrency(t *testing.T) {
	server, client, serverTCP, clientTCP := setupRPCServerAndClient(t)
	...
	// 统计成功和失败
	success := len(successCount)
	errorList := make([]error, 0, len(errors))
	for err := range errors {
	...
	// 验证所有请求都成功
	if len(errorList) > 0 {
		t.Fatalf("%d out of %d requests failed:\n%v", len(errorList), numRequests, errorList)
	}

	t.Logf("✅ High concurrency test passed: %d/%d requests succeeded in %v (%.2f req/s)",
		success, numRequests, duration, float64(success)/duration.Seconds())
}
